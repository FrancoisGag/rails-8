...
class ProductTest < ActiveSupport::TestCase
  ...
  test "invalid without title" do
    @product.title = ""
    assert_not @product.valid?, "saved product without a title"
  ...
  test "invalid with a taken title" do
    ...
    assert_not same_title.valid?, "saved product without a title"
  ...
  test "invalid without image" do
    ...
    assert_not without_image.valid?, "saved product without an image"
    assert_equal [ "can't be blank" ], without_image.errors[:image]
  end

  test "invalid without price" do
    @product.price = nil
    assert_not @product.valid?, "saved product without a price"
    assert_equal [ "is not a number" ], @product.errors[:price]
  end

  test "invalid with NaN price" do
    @product.price = "price"
    assert_not @product.valid?, "saved product price with string"
    assert_equal [ "is not a number" ], @product.errors[:price]
  end

  test "invalid with price <= 0" do
    @product.price = 0
    assert_not @product.valid?, "saved product price with 0"
